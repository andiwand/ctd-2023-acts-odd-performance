...
for file in args.input:
    ...
    true_pt, res_qop = get_data(file)

    qop_std, pt_edges, _ = binned_statistic(
        true_pt, res_qop, bins=pt_bins, range=pt_range, statistic=smoothed_std
    )
    qop_std_std, pt_edges, _ = binned_statistic(
        true_pt, res_qop, bins=pt_bins, range=pt_range, statistic=smoothed_std_std
    )
...
plt.title(rf"Resolution of $\frac{{q}}{{p}}$ over $p_T$")
plt.xlabel(r"$p_T$")
